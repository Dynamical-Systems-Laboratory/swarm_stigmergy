...
% Modulation of k
k_mod = k+A*sin(Omega*(t_vec(t_ind:end)'-t_vec(t_ind))-pi);

% Construction of the objective density
rho_obj(:,1:t_ind-1) = repmat(rho_obj_in,[1,t_ind-1]);
rho_obj(:,t_ind:end) = exp(k_mod.*cos(mesh-mu-v*(t_vec(t_ind:end)'-t_vec(t_ind))))./(2*pi*besseli(0,k_mod));
...
% Export
writematrix(exp_profiles,'Dynamic_modulating_profiles.txt','Delimiter','tab');
writematrix(exp_profile_obs,'Dynamic_modulating_profile_obs.txt','Delimiter','tab');
writematrix(exp_KL_div,'Dynamic_modulating_KL_div.txt','Delimiter','tab');
writematrix(exp_colorplot,'Dynamic_modulating_colorplot.txt','Delimiter','tab');