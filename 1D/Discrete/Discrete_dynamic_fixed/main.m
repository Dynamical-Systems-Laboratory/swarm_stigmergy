...
rho_obj = M*exp(k*cos(mesh-mu+v*[t_vec;t_vec(end)+dt]'))./(2*pi*besseli(0,k)); % Objective density
rho_obj_t = (rho_obj(:,2:end)-rho_obj(:,1:end-1))/dt; % Time derivative of objective density
...
writematrix(exp_KL,'Dynamic_fixed_KL.txt','Delimiter','tab');
